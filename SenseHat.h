...
public:
    ...
    void AfficherMessage(std::string message,int vitesseDefilement=1000,uint16_t CouleurTexte = BLEU,uint16_t couleurFond = NOIR);
...
private:
    ...
    // Converti un caractère en Motif affichable sur la matrice de leds - par Grilo Christophe
    void ConvertirCaractereEnMotif(char c,uint16_t image[8][8],uint16_t couleurTexte, uint16_t couleurFond);
    // Permet de savoir si une colonne de leds est vide - par Grilo Christophe
    bool ColonneVide(int numColonne,uint16_t image[8][8],uint16_t couleurFond);
    // Permet de supprimer une colonne sur un ensemble de motif (image) - par Grilo Christophe
